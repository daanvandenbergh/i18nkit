...
- **The entire blog area when the project uses `@daanvandenbergh/blogkit`**: MDX article bodies
  ...
  **file-per-language** by blogkit - each default-locale post at `<contentDir>/<slug>.mdx` gets a
  same-slug translation under `<contentDir>/<code>/` (e.g. `blog/nl/<slug>.mdx`), never wrapped in a
  translator. Do not flag literals that are article content or come from a post's front-matter. (The
  blog's own page-shell chrome - a hardcoded nav label or empty state in a route/component - would
  still be copy, but the blog area is content-dominated; treat it as the low-yield exception.)
  Translation *completeness* is not a translator concern and is checked separately by the sweep's
  blogkit-parity step, not flagged here.
