...
description: Add one new locale to a project already built on i18nkit and carry every translation through - the compiler-guided counterpart to i18nkit-sweep. Adding the code to `new I18n({ locales })` grows the locale union `L`, so `tsc` turns every `defineTextCatalog` / `defineText` / `LanguageText` entry still missing that locale into a compile error: an exhaustive, exact worklist of what to translate, handed to you by the compiler. This skill edits the I18n config, then translates every erroring entry - preserving interpolation params, `${...}` placeholders, split rich-text fragments, and leaving brand / `i18n.uniform` words untouched - fanning the work across parallel agents when there are many, and loops `tsc` to green. It also covers what the compiler cannot see: for `@daanvandenbergh/blogkit` it enumerates and creates the same-slug translated post for every default-locale article (front-matter + body, hero preserved), and it hunts hardcoded locale lists (static params, hreflang, sitemaps, middleware) that name codes literally. Use this whenever the user wants to add, support, or ship a new language or locale, translate the whole site / app into another language, internationalize into German / French / Spanish / Japanese / etc., launch in a new country or market, or asks "how do we add <language>?" - even if they never name i18nkit. It EXTENDS an already-wrapped codebase to a new locale; if bare untranslated strings might still exist, run i18nkit-sweep FIRST so the compiler can see them, then this. Takes the new locale code and an optional endonym label.
user-invokable: true
...
`contentDir`/`extension` from the config if the defaults differ):

```bash
EXISTING_CODES="en nl" CONTENT_DIR="./blog" <skill-dir>/scripts/scan.sh <newcode> "<src or .>"
```

It prints two lead sections - confirm each by reading, none is a verdict:
1. **BLOGKIT POST WORKLIST** - for each default-locale post, the `<contentDir>/<newcode>/<slug>` file
   that must exist, and whether the base post carries a hero `image:`. If blogkit is in use: add the
   new locale to the `Blog` config's `locales[]`, then create each listed post - **same slug**,
   front-matter human fields + body translated per translation-rules.md, hero carried over. Posts are
   independent, so fan them out one-agent-per-post exactly like Phase 3 when there are several; for
   long articles, hand the precise worklist to the author and point at blogkit's own writing / hero
   skills rather than machine-translating a long body. Report any **hero gap** or **broken hero
   reference**. If blogkit is not detected, say so and skip.
...
2. **Report**, in the conversation, per `reference/translation-rules.md`'s "What done looks like":
   ...
   - the **`needs-review` list** - every flagged entry by `file:line` + reason, so a human reviews only
     those;
   - the **blogkit result** (posts created / handed off, hero gaps, or "not detected");
